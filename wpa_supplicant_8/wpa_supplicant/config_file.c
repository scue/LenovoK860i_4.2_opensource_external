...
static void write_freq_list(FILE * f, struct wpa_ssid *ssid)
{

    char *value = wpa_config_get(ssid, "freq_list");
	wpa_printf(MSG_INFO,"freq_lsit is %s",value?value:"null");
	if (value == NULL)
		return;
	fprintf(f, "\tfreq_list=%s\n", value);
	wpa_printf(MSG_INFO,"freq_lsit is %s",value);
	os_free(value);
}
