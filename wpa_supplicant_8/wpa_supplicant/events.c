					 ...
					 struct wpa_bss *bss)
{
	...
	int ret;
	int wep_ok;
	wpa_printf(MSG_ERROR, "%s : Enter\n",__func__);
	ret = wpas_wps_ssid_bss_match(wpa_s, ssid, bss);
	if (ret >= 0)
		return ret;
	wpa_printf(MSG_ERROR, "%s : get ret @@@@",__func__);
#ifdef WAPI
	wapi_ie = wpa_bss_get_ie(bss, WLAN_EID_WAPI);
	if(wapi_ie)
		wpa_printf(MSG_ERROR, "%s : get wapi_ie\n",__func__);
	else
		wpa_printf(MSG_ERROR, "%s : wapi is null\n",__func__);
	while ((ssid->proto & WPA_PROTO_WAPI) && wapi_ie)
	{
		...
		wpa_printf(MSG_DEBUG, "ssid->wapi=%x, ssid->key_mgmt=%x "
				      "bss->wapi_ie_len=%d \n"
				, ssid->wapi, ssid->key_mgmt, bss->wapi_ie_len);
...
#ifdef WAPI
		wpa_dbg(wpa_s, MSG_DEBUG, "   skip - no WPA/RSN/WAPI proto match");
#else
			  ...
			  struct wpa_ssid **selected_ssid)
{
	...
	wpa_dbg(wpa_s, MSG_ERROR, "Selecting BSS from priority group %d",
		group->priority);
#ifdef WAPI
	struct wpa_scan_res *selected = NULL;
	wpa_printf(MSG_DEBUG, " * %s",__func__);
	struct wpa_ssid *ssid;
	const u8 *ssid_, *ie, *ie2;
	u8 ssid_len, wapi_ie_len;
	struct wpa_blacklist *e;

	/* First, try to find WAPI-enabled AP */
	wpa_printf(MSG_DEBUG, " Try to find WAPI-enabled AP");
	for (i = 0; i < wpa_s->last_scan_res_used /*scan_res->num*/ && !selected; i++)
	{
		struct wpa_bss /*wpa_scan_res*/ *bss = wpa_s->last_scan_res[i]; //scan_res->res[i];
		...
		*/
		...
		u8 *wapi_ie = wpa_bss_get_ie(bss, WLAN_EID_WAPI);
		u8 wapi_ie_len = wapi_ie ? wapi_ie[1] + 2: 0;
		...
		if (wapi_ie_len == 0) {
			...
			wpa_printf(MSG_DEBUG, " * get WAPI AP via WAPI_IE, wapi_ie_len = %d wapi_ie_len = %d\n",bss->wapi_ie_len, wapi_ie_len);

		for (ssid = group; ssid; ssid = ssid->pnext)
		{
			wpa_printf(MSG_DEBUG, " * [WAPI] Enter for loop\n");
			if (ssid->disabled) {
				wpa_printf(MSG_DEBUG, " [WAPI] skip - disabled");
				continue;
			}
			if (ssid_len != ssid->ssid_len ||
			    os_memcmp(ssid_, ssid->ssid, ssid_len) != 0) {
				wpa_printf(MSG_DEBUG, " [WAPI] skip - "
					   "SSID mismatch");
				continue;
			}
			if (ssid->bssid_set &&
	...
	if (selected)
	{
		wpa_printf(MSG_DEBUG, " [WAPI] select WAPI AP");
		...
		*/
		...
		wpa_printf(MSG_DEBUG, "NO selected WAPI BSS");
#endif
...
int wpas_select_network_from_last_scan(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_ERROR, "## %s: Enter\n",__func__);
	if (selected) { 
		wpa_printf(MSG_ERROR, "## %s: Get the suitable network\n",__func__);
		int skip;
		...
		wpa_dbg(wpa_s, MSG_ERROR, "## No suitable network found");
...
#endif /* CONFIG_AP */
#ifdef WAPI
	wpa_printf(MSG_DEBUG,"%s: associated to a wapi network.\n", __FUNCTION__);
