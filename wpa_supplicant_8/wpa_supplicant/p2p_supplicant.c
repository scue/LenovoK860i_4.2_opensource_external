...
static void wpas_update_freq_list(struct p2p_data *p2p, struct wpa_ssid * ssid, const char  *mac)
{
	...
	#define MAXRECORD_CHANNEL (30)
	int freqlist[MAXRECORD_CHANNEL+1];
	if( p2p == NULL || ssid== NULL || mac== NULL) {
             goto exit;
	}
	wpa_printf( MSG_DEBUG, "wpas_update_freq_list : " MACSTR, MAC2STR(mac));
	if(ssid->num_p2p_clients !=1) {
	      wpa_printf( MSG_DEBUG, "wpas_update_freq_list : will not update freq list");
             goto exit;
	}

	memset(freqlist,0,sizeof(freqlist));
        dev = p2p_get_device(p2p, mac);
	if(dev==NULL) {
             wpa_printf( MSG_DEBUG, "can't find device  : " MACSTR, MAC2STR(mac));
	      goto exit;
	}
	p2p_channels_intersect(&(p2p->cfg->channels), &dev->channels, &intersection);
	if (intersection.reg_classes == 0 ||
	intersection.reg_class[0].channels == 0) {
		wpa_printf( MSG_DEBUG, "P2P: wpas_update_freq_list No common channels found");
		goto exit;
	}
	...
	{
		int j=0;
		if(channels>=MAXRECORD_CHANNEL) break;
		for(j=0;j<intersection.reg_class[i].channels;j++)
		{
			if(channels>=MAXRECORD_CHANNEL) break;
			freq=p2p_channel_to_freq("xx\x4",intersection.reg_class[i].reg_class,
				intersection.reg_class[i].channel[j] );
                        if( freq>2411) {
                             freqlist[channels++]=freq;
                        }
		 }
	}
	if( channels ) {
               os_free(ssid->freq_list);
	       ssid->freq_list=os_malloc(sizeof(int)*(channels+1));
	       if( ssid->freq_list ) {
		      ssid->freq_list[channels]=0;
                      memcpy(ssid->freq_list ,freqlist,sizeof(int)*channels);
		 }
	}
...
#ifndef CONFIG_NO_CONFIG_WRITE
    wpas_update_freq_list(wpa_s->global->p2p,s,addr);
