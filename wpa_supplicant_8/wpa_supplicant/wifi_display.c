...
int wifi_display_inuse(void * supplicant)
{
       int i=0;
       int ret=0;
       struct wpa_global *global;
       struct wpa_supplicant * ifs;
       struct wpa_supplicant * p2psupplicant;
       p2psupplicant=(struct wpa_supplicant *)supplicant;
       if(!supplicant || !p2psupplicant->global) 
        {
             wpa_printf(MSG_DEBUG,"wifi_display_inuse: supplicant is null");
             return 0;
        }

       global= p2psupplicant->global;
        
       wpa_printf(MSG_DEBUG,"wifi_display_inuse: wifi_display_inuse");
       if( global->wifi_display)
       {
               wpa_printf(MSG_DEBUG,"wifi_display_inuse: wifi_display is enabled. ifs %d",ret);
               for(i=0;i< MAX_WFD_SUBELEMS;i++)
               {
                       if(global->wfd_subelem[i])
                       {   wpa_printf(MSG_DEBUG,"wifi_display_inuse: wfd_subelem is not empty");
                            ret=1;
                            break;
                       }
               }
       }
       wpa_printf(MSG_DEBUG,"wifi_display_inuse: checking1 ifs %d",ret);
       if( ret)
       {
               wpa_printf(MSG_DEBUG,"wifi_display_inuse: checking ifs %d",ret);
               ret=0;
               
               for(ifs=global->ifaces;ifs;ifs=ifs->next)
               {
                    wpa_printf(MSG_DEBUG,"wifi_display_inuse: checking %s ",ifs->ifname);
                    if( memcmp(ifs->ifname,"p2p-",4)==0)
                    {
                         if( ifs->wpa_state == WPA_COMPLETED )
                         {
                                 wpa_printf(MSG_DEBUG,"wifi_display_inuse: %s is working",ifs->ifname);
                                 ret=1;
                                 break;
       ...
       wpa_printf(MSG_DEBUG,"wifi_display_inuse: wifi_display_inuse return %s",ret?"in use" : " not inuse");
       return ret;
}
