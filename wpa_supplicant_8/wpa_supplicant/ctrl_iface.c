	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	const u8 *ie3;
#endif
	wpa_printf(MSG_DEBUG,"## cmd from wpa_cli %s",__func__);
...
#ifdef WAPI
	ie3 = wpa_scan_get_ie(bss, WLAN_EID_WAPI);
	if (ie3)
	{
		wpa_printf(MSG_DEBUG,"## get WAPI");
		...
		wpa_printf(MSG_DEBUG,"## get not WAPI");
#endif
...
static void  _p2p_select_channel(int * channelist , unsigned int number, int * randomone)
{
	unsigned int channel;
	struct os_time t;
	os_get_time(&t);
	os_get_random(&channel,sizeof(channel));
	channel +=(unsigned int) t.sec;
	channel = channel%number;
	*randomone=channelist[channel];
}
int p2p_random_operate_channel(int * channellist,int * rcvchannel)
{
	int band2G[13];
	#define MAXCHANEL (100)
	int band5G[MAXCHANEL];
	int i=0;
	int band2Gpos=0;
	int band5Gpos=0;
	int freq=0;
	if(channellist==NULL || rcvchannel==NULL)
		return -1;
	for(i=0; channellist[i] && i<MAXCHANEL;i++) {
		freq=channellist[i];
		if(freq<=2462) {
			if(2412 ==freq ||
			   2437 ==freq ||
		...
		if( band2Gpos < (sizeof(band2G)/sizeof(int)))  
				band2G[band2Gpos++]=freq;
		} else if( freq>5000 && freq<=7000) {
			if( band5Gpos < (sizeof(band5G)/sizeof(int)))
				band5G[band5Gpos++]=freq;
		}
	}
	if( band5Gpos ) {
	        _p2p_select_channel(band5G,band5Gpos,rcvchannel);
	} else if(band2Gpos) {
	       _p2p_select_channel(band2G,band2Gpos,rcvchannel);
	}
	return 0;
}
static int p2p_ctrl_invite_persistent(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	{
		int myoperatechannel=-1;
		p2p_random_operate_channel(ssid->freq_list,&myoperatechannel);
		if(myoperatechannel >0)
		{
			p2p_freq_to_channel("xx\x4", myoperatechannel,
						&wpa_s->global->p2p->op_reg_class, &wpa_s->global->p2p->op_channel);
			wpa_printf(MSG_DEBUG, "p2p_ctrl_invite_persistent, op_reg_class[%d], op_channel[%d] ",
					  wpa_s->global->p2p->op_reg_class, wpa_s->global->p2p->op_channel);
		}
	}
