...
static const char *wapi_cert_name = "/data/misc/wifi/wapi_merge.cer";



/* initialize the wapi_config structure from a wpa_ssid structure */
struct wapi_config * wapi_config_init(struct wpa_ssid *ssid)
{
	...
	if(eloop->ifaces != NULL)
		wpa_printf(MSG_DEBUG, "%s : eloop->iface =======> %p\n",__func__ ,eloop->ifaces);
	
	if(eloop->ifaces->next != NULL) {
		eloop->ifaces = eloop->ifaces->next;
		wpa_printf(MSG_DEBUG, "%s : eloop->iface->next =======> %p\n",__func__ ,eloop->ifaces);
	}
	...
	wpa_printf(MSG_DEBUG, "%s : wapi_policy '%d'",__func__, config->wapi_policy);

	if (config->wapi_policy & 0x08)
	{
		wpa_printf(MSG_DEBUG, "%s : AUTH_TYPE_WAPI",__func__);
		lib_param.authType = AUTH_TYPE_WAPI;
	} else if (config->wapi_policy & 0x04)
	{
		wpa_printf(MSG_DEBUG, "%s : AUTH_TYPE_WAPI_PSK",__func__);
		lib_param.authType = AUTH_TYPE_WAPI_PSK;
	} else {
		wpa_printf(MSG_DEBUG, "%s : AUTH_TYPE_NONE_WAPI",__func__);
	...
	{
#ifdef WAPI
		wpa_printf(MSG_DEBUG, "%s : WAPI_CIPHER_SMS4",__func__);
	...
	config->ssid = (u8 *)strdup(ssid->ssid);
	...
	if (config->wapi_policy & 0x08) {
		if ( (ssid->wapi_as_cert) && (ssid->wapi_user_cert) )
		{
			...
			ssid->cert_name = malloc(sizeof(eloop->cert_info.config.cert_name));
			if (ssid->cert_name == NULL) {
				wpa_printf(MSG_ERROR, "%s: malloc(%d) failed, exit...\n",
					   __FUNCTION__, sizeof(eloop->cert_info.config.cert_name));
				wapi_config_free(config);
				return NULL;
			}

			/* create merged cert using the specific file name */
			strcpy(ssid->cert_name, wapi_cert_name);
			...
			wpa_printf(MSG_ERROR, "%s: wapi cert mode but no enough cert files, quit association\n",
				   __FUNCTION__);
			wapi_config_free(config);
			return NULL;
		}
	...
	{ /* bit 2:PSK */
		...
		if (ssid->passphrase != NULL) {
			wpa_printf(MSG_DEBUG, "%s: ssid->passphrase !== NULL, took ssid->passphrase %s as wapi psk\n",
		...
		else if (ssid->psk != NULL) {
			wpa_printf(MSG_DEBUG, "%s: ssid->passphrase == NULL, took ssid->psk %s as wapi psk\n",
				   __FUNCTION__, ssid->psk);
			lib_param.para.kl = strlen(ssid->psk);
			memcpy(lib_param.para.kv, ssid->psk, lib_param.para.kl);
		} else {
			wpa_printf(MSG_ERROR, "%s: passphrase and psk both NULL, exit association...\n",
				   __FUNCTION__);
			wapi_config_free(config);
			return NULL;
		}
	...
	wpa_printf(MSG_DEBUG, "%s: before WAI_CNTAPPARA_SET\n", __FUNCTION__);
...
{
#ifdef WAPI
	static u8 ssid_buf[MAX_SSID_LEN+1];
	u8 *wapi_ie;
 	u8 wapi_ie_len;
#endif
	u8 wpa_ie[200];
	...
	if ( (ssid->wapi) || (ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ) ||
		(ssid->key_mgmt == WPA_KEY_MGMT_WAPI_CERT ) ) { /* associating to a WAPI network */
        struct wpa_global *global;

		        global = (struct wpa_global *)eloop_get_user_data();
			
		wpa_printf(MSG_DEBUG, "\nWAPI Info : ssid->wapi %d ssid->proto %d ssid->key_mgmt %d \n", ssid->wapi, ssid->proto, ssid->key_mgmt);
		...
		if (ssid->wapi == 0) {
			if (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK) {
				...
				wpa_printf(MSG_ERROR, "%s: wpa.conf lack of information whether %s is a WAPI PSK or WAPI CERT network, can't associate to it.\n", __FUNCTION__, ssid->ssid);
				return;
			}
		}
		if (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK) {
				...
				params.key_mgmt_suite = KEY_MGMT_WAPI_CERT;
		}
		wpa_s->wapi_conf = wapi_config_init(ssid);

		if (wpa_s->wapi_conf == NULL) {
			wpa_printf(MSG_ERROR, "%s: initialize wapi_conf failed, exit association to WAPI network\n",
				   __FUNCTION__);
			return;
		}

		wpa_printf(MSG_DEBUG, "%s: params cleared\n", __FUNCTION__);

		if (bss) {
			u8 *ie = wpa_bss_get_ie(bss, WLAN_EID_SSID);
			if(ie) {
				wpa_printf(MSG_DEBUG, "WLAN_EID_SSID Get ie: [%02x][%02x][%02x][%02x][%02x][%02x][%02x][%02x][%02x][%02x]\n",
		                           ie[0], ie[1], ie[2], ie[3], ie[4],
		                           ie[5], ie[6], ie[7], ie[8], ie[9] );

				wpa_printf(MSG_DEBUG, "%s: bss enter, ie[1]=%d\n", __FUNCTION__, ie[1]);
				params.bssid = bss->bssid;
				params.ssid = ssid_buf;

				if ( ie[1] >  MAX_SSID_LEN ) {
					wpa_printf(MSG_ERROR, "%s: ssid length is over %d, strip it!", __FUNCTION__, MAX_SSID_LEN);
				}

				snprintf( params.ssid, ((ie[1]>MAX_SSID_LEN)?MAX_SSID_LEN:ie[1])+1, "%s", ie ? wpa_ssid_txt(ie + 2, ie[1]) : "");


				params.ssid_len = strlen(params.ssid);

				params.freq = bss->freq;
				wpa_printf(MSG_DEBUG, "%s: params.ssid=%s\n", __FUNCTION__, params.ssid);
				wpa_printf(MSG_DEBUG, "%s: params.bssid=%s\n", __FUNCTION__, params.bssid);
				wpa_printf(MSG_DEBUG, "%s: bss done\n", __FUNCTION__);
		...
		params.wpa_ie = wpa_s->assoc_wapi_ie; /* to do: do I have to manually copy the contents into params.wpa_ie? */

                wpa_printf(MSG_DEBUG, "wapi_ie: [%02x][%02x][%02x][%02x]\n",
                   		params.wpa_ie[0], params.wpa_ie[1], params.wpa_ie[2], params.wpa_ie[3]);

		if(bss) {
		        u8 *ap_wapi_ie = wpa_bss_get_ie(bss, WLAN_EID_WAPI);
			if(ap_wapi_ie) {
				params.ap_wapi_ie = ap_wapi_ie;
				params.ap_wapi_ie_len = ap_wapi_ie ? ap_wapi_ie[1] + 2: 0;
#if 0
                params.wpa_ie = params.ap_wapi_ie;
		params.wpa_ie_len = params.ap_wapi_ie_len;
		wpa_printf(MSG_DEBUG, "wapi_ie: [%02x][%02x][%02x][%02x]\n",
				                                params.wpa_ie[0], params.wpa_ie[1], params.wpa_ie[2], params.wpa_ie[3]);
#endif		
				wpa_printf(MSG_DEBUG, "%s: params.ap_wapi_ie_len = %d\n", __FUNCTION__, params.ap_wapi_ie_len);
				wpa_printf(MSG_DEBUG, "%s: params.ap_wapi_ie_len: [%02x][%02x][%02x][%02x]\n",
		                        __FUNCTION__, params.ap_wapi_ie[0], params.ap_wapi_ie[1], params.ap_wapi_ie[2], params.ap_wapi_ie[3]);
		...
		if (!memcmp(wpa_s->bssid, "\x00\x00\x00\x00\x00\x00", ETH_ALEN)) {
			...
			wpa_printf(MSG_DEBUG, "call wpa_drv_associate\n");
				     ...
				     struct wpa_interface *iface)
{
	...
	if (iface->confname) {
		...
		wpa_printf(MSG_ERROR, "interface  %s\n",iface->ifname);
                if(!strcmp(iface->ifname,"wlan0"))
		{					
			wpa_printf(MSG_ERROR, " Enter WIFI_lib_init \n\n");
			if (WIFI_lib_init()) {
				...
				return -1;
                	...
                	wpa_printf(MSG_ERROR, "\n--------------------------------------------\n");
                        wpa_printf(MSG_ERROR, "|   Failed to initialize WAPI lib, exit    |\n");
		        wpa_printf(MSG_ERROR, "|   Check your wireless interface,         |\n");
			wpa_printf(MSG_ERROR, "|   defaulshould be wlan0                  |\n");
			wpa_printf(MSG_ERROR, "--------------------------------------------\n");
		}					      			  
#endif
		if (wpa_s->conf == NULL) {
