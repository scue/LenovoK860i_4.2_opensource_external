...
int p2p_select_random_operation_channel(struct p2p_channels *channs, int *index_calss, int *index_channel)
{
        struct p2p_channels * channel_list = channs;
	int reg_class = 0;
        int reg_channel = 0;
        int tries = 0;

	if(!channel_list->reg_classes) return -1;
	wpa_printf(MSG_DEBUG,"p2p_select_random_operation_channel, reg_classes[%d] ",channel_list->reg_classes);
        while( tries ++ < 10 &&channel_list->reg_classes > 1) {
		os_get_random(&reg_class, sizeof(reg_class));
		reg_class = reg_class % channel_list->reg_classes;
		if(channel_list->reg_class[reg_class].reg_class != 81)
		break;
	}

	os_get_random(&reg_channel, sizeof(reg_channel));
	reg_channel = reg_channel % channel_list->reg_class[reg_class].channels;
	if (channel_list->reg_class[reg_class].reg_class == 81) {
		if( (1 == channel_list->reg_class[reg_class].channel[reg_channel]) ||
		    (6 == channel_list->reg_class[reg_class].channel[reg_channel]) ||
		    (11 ==channel_list->reg_class[reg_class].channel[reg_channel]) ) {
                             reg_channel=8;
		}
	}

        *index_calss = reg_class;
        *index_channel = reg_channel;
	wpa_printf(MSG_DEBUG, "P2P: select random oprate channel reg_class[%d] = %d,  reg_channel[%d] = %d ",
			*index_calss, channel_list->reg_class[reg_class].reg_class, *index_channel,
			channel_list->reg_class[reg_class].channel[reg_channel]);
        return 0;
}
		...
		int pd_before_go_neg)
{
	...
	if (force_freq)
		...
		dev->flags &= ~P2P_DEV_FORCE_FREQ;
                ...
                if(p2p_select_random_operation_channel(&p2p->channels, &reg_class, &reg_channel) >= 0){
                p2p->op_reg_class = p2p->channels.reg_class[reg_class].reg_class;
                p2p->op_channel = p2p->channels.reg_class[reg_class].channel[reg_channel];
                }
        }
...
void p2p_scan_res_handled(struct p2p_data *p2p)
{
	...
	if (p2p_run_after_scan(p2p))
		return;
	if (p2p->state == P2P_SEARCH){
                if(wifi_display_inuse(p2p->cfg->msg_ctx))
                   return; 
		p2p_continue_find(p2p);
        } 
}
