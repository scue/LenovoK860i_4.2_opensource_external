			  ...
			  struct p2p_channels *intersection)
{
	 ...
	 */
        ...
        if(!p2p_select_random_operation_channel(intersection, &index_calss, &index_channel)){
		cl = &intersection->reg_class[index_calss];
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Pick another channel "
		"(reg_class %u channel %u) from intersection",
		cl->reg_class, cl->channel[index_channel]);
		p2p->op_reg_class = cl->reg_class;
		p2p->op_channel = cl->channel[index_channel];
	} else {
